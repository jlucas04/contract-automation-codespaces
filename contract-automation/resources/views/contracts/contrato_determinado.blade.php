<!-- resources/views/contracts/contrato_determinado.blade.php -->
<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <title>Contrato Determinado</title>
    <style>
        body {
...
<h2>CONTRATO DE TRABALHO - DETERMINADO</h2>
...
<div class="clause"><strong>1ª – DO OBJETO:</strong> Prestação de serviços temporária, conforme necessidade do EMPREGADOR.</div>
<div class="clause"><strong>2ª – DA FUNÇÃO:</strong> {{ $collaborator->role }}.</div>
<div class="clause"><strong>3ª – REMUNERAÇÃO:</strong> R$ {{ number_format($collaborator->salary,2,',','.') }} mensais.</div>
<div class="clause"><strong>4ª – DURAÇÃO:</strong> Início em {{ \Carbon\Carbon::now()->format('d/m/Y') }}, término em {{ $collaborator->end_date ? \Carbon\Carbon::parse($collaborator->end_date)->format('d/m/Y') : '____/____/____' }}.</div>
<div class="clause"><strong>5ª – DISPOSIÇÕES FINAIS:</strong> Foro da comarca de {{ $collaborator->city }}/{{ $collaborator->state }}.</div>
